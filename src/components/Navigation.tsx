    ...
    <nav className="sticky top-0 z-50 bg-white shadow-md font-sans border-b border-green-100">
      <div className="container mx-auto px-2">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2 lg:space-x-2">
            <img src="images/Logo unit180.png" alt="Unit 180 Logo" className="w-10 h-10 object-contain rounded bg-white shadow-lg p-1 ring-2 ring-green-400 ring-offset-2 ring-offset-white transition-all duration-300 hover:scale-105 hover:shadow-green-400/50" />
            <div className="leading-tight">
              <div className="text-green-700 font-extrabold text-base lg:text-lg tracking-tight drop-shadow-md">UNIT 180</div>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className={`relative font-semibold text-base uppercase tracking-wide px-3 py-2 rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 focus:ring-offset-white text-green-700 hover:text-green-900 ${isActive(item.path) ? 'font-bold' : ''}`}
              >
